...
import com.cubeia.poker.variant.texasholdem.StudHandCalculator;
 ...
 * @author w
 */
public class DefaultBestHandPlayerToActCalculator implements PlayerToActCalculator {

    private static final long serialVersionUID = 1470908665729381611L;

    private StudHandCalculator evaluator;



    public DefaultBestHandPlayerToActCalculator() {
        evaluator = new StudHandCalculator();
    }
    ...
    public PokerPlayer getFirstPlayerToAct(SortedMap<Integer, PokerPlayer> seatingMap, List<Card> communityCards) {
        ...
        for (PokerPlayer p : seatingMap.values()) {
            ...
            if(currentBestHand == null  || thc.compare(pHand, currentBestHand)>0) {
                currentBestPlayer = p;
